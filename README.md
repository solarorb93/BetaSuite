...
These are the installation guidelines for BetaSuite v0.1.1-rc2.  At this time only BetaTest, BetaStare, and BetaTV are supported, with BetaVision coming in a later release (it is working and fully functional, it just needs work to get it into a easy-to-use state).
...
- Complete the Download BetaSuite section below to download v0.1.1-rc2.
...
6. Download BetaSuite
    * Download BetaSuite v0.1.1-rc2 source code zip file from this link: https://github.com/solarorb93/BetaSuite/releases/tag/v0.1.1-rc2
    * Unzip the files into InstallFolder/BetaSuite-0.1.1-rc2/ (so you should have a file InstallFolder/BetaSuite-0.1.1-rc2/betastare.py)
    * Open a cmd window by going to Start->Run and entering `cmd` and then pressing enter.  In the black window that appears, type the following commands, pressing enter after each one:
        ...
        * `cd InstallFolder/BetaSuite-0.1.1-rc2/` where InstallFolder is your InstallFolder
        * `python betatest.py`
...
* Navigate to InstallFolder/BetaSuite-0.1.1-rc2/ and open betaconfig.py in Notepad by right-clicking on it, hovering over "Open With", and choosing Notepad.  In the first section of the file, change gpu_enabled=0 to `gpu_enabled=1`.
* Open a cmd window by going to Start->Run and entering `cmd` and then pressing enter.  In the black window that appears, type the following commands, pressing enter after each one:
    * `c:` where c is the letter of the drive that InstallFolder is located on
    * `cd InstallFolder/BetaSuite-0.1.1-rc2/` where InstallFolder is your InstallFolder
    * `python betatest.py`
...
* If for any reason you cannot make GPU Acceleration work, you need to go back to the original CPU-only setup, by doing the following steps
    * Open a cmd window by going to Start->Run and entering `cmd` and then pressing enter.  In the black window that appears, type the following commands, pressing enter after each one:
        ...
        * `pip install onnxruntime`
    * Navigate to InstallFolder/BetaSuite-0.1.1-rc2/ and open betaconfig.py in Notepad by right-clicking on it, hovering over "Open With", and choosing Notepad.  In the first section of the file, change gpu_enabled=1 to `gpu_enabled=0`. Then save and close the file.
...
You can adjust how the censoring works by modifying InstallFolder/BetaSuite-0.1.1-rc2/betaconfig.py.  
Navigate to InstallFolder/BetaSuite-0.1.1-rc2/ and open betaconfig.py in Notepad by right-clicking on it, hovering over "Open With", and choosing Notepad.
